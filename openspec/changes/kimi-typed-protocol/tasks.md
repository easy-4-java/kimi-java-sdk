...
## 1. 门禁与 schema 证据
- [x] 1.1 [TYP-01..06] 评审本 Change 并固定已验证 wire fixtures；未确认字段保持 raw。
- [x] 1.2 执行 `openspec validate kimi-typed-protocol --strict`。
- [x] 1.3 建立 CLI/ACP/Server fixture 清单，标注来源和字段稳定性。
...
- [x] 2.4 [TYP-05] 写 typed/raw 一致性测试。
- [x] 2.5 [TYP-06] 建立跨 Jackson fixture contract。
...
- [x] 3.5 [TYP-06] 分别实现 Jackson 2/3 adapter。

## 4. REFACTOR
- [x] 4.1 去除 transport/domain 的重复解析逻辑，保持依赖单向。
- [x] 4.2 审查 equals/hashCode/toString，确保 toString 不泄露 token/secret/完整敏感内容。

## 5. 三线验证
- [x] 5.1 feature/3.0.x fixtures + unit tests。
- [x] 5.2 feature/2.0.x 同 fixtures。
- [x] 5.3 feature/1.0.x 同 fixtures。
- [x] 5.4 比较三线 serialized/parsed semantic snapshots，无未解释差异。
